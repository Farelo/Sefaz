...
const parserMessage = async (jsonMessage, deviceData) => {
   ...
   let auxTimestamp = 0;
   if (jsonMessage.timestamp.toString().length == 10) auxTimestamp = jsonMessage.timestamp;
   else if (jsonMessage.timestamp.toString().length == 13) auxTimestamp = jsonMessage.timestamp / 1000;
   ...
   deviceData.message_date = new Date(moment(auxTimestamp * 1000).subtract(3, "h"));
   deviceData.message_date_timestamp = auxTimestamp;
