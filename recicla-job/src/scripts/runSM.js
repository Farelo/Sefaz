...
// EVALUATORS
const evaluatesCurrentControlPoint = require('./evaluators/evaluates_current_control_point')

// WORKFLOWS
const withoutRoute = require('./workflows/without_routes')

module.exports = async (setting, packing) => {
    try {
        // TODO: No atributo packing.last_device_data ele já irá trazer o last_device_data filtrado pela accurancy?
        const lastDeviceData = packing.last_device_data ? packing.last_device_data : null
            ...
            case undefined:
                if (getDiffDateTodayInDays(lastDeviceData.last_communication) > setting.no_signal_limit_in_days) {
                    ...
                    * Está em alguma planta atualmente? */
                    const currentControlPoint = await evaluatesCurrentControlPoint(packing, controlPoints, setting)

                    // await withoutRoute(packing, currentControlPoint)
                ...
                break
            // case packing.last_alert_history.type === 'missing'
        }
    } catch (error) {
        console.error(error)
