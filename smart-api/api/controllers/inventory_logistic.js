...
exports.general_inventory_packing = function (req, res) {
  ...
  const aggregate = schemas.packing.aggregate(query.queries.inventory_general(map));
...
exports.quantity_inventory = function (req, res) {
  const map = req.body.map(o => new ObjectId(o));
  const aggregate = schemas.packing.aggregate(
    query.queries.quantity_inventory(req.swagger.params.code.value, map),
  );
...
exports.inventory_battery = function (req, res) {
  ...
  schemas.packing
    .paginate(code ? { supplier: { $in: map }, code } : { supplier: { $in: map } }, {
      page: parseInt(req.swagger.params.page.value),
...
exports.inventory_packing_absence = function (req, res) {
  ...
  schemas.historicPackings
    .paginate(
      { supplier: { $in: map }, serial, packing_code: code },
      {
        page: parseInt(req.swagger.params.page.value),
