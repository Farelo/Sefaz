        ...
        if (packing.traveling) {
          if (packing.trip.time_exceeded) {
            ...
            if (delayedRoutes.length > 0) {
              debug(`THIS PACKING IS LATE, WHE'RE CREATING A ALERT TO: ${packing._id}`)        
              packing.trip.time_exceeded = true
              packing.trip.time_countdown = timeInterval
              module.exports.updateTravelingAlert(packing).then(new_packing => resolve(new_packing))
            } else {
              debug(`THIS PACKING IS OKAY: ${packing._id}`)
