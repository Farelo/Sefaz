...
const validate_companies = (company) => {
  const schema = Joi.object().keys({
    ...
    address: {
      street: Joi.string().max(140).allow(null, ""),
      numero: Joi.string().max(10).allow(null, ""),
      city: Joi.string().max(80).allow(null, ""),
      uf: Joi.string().max(2).allow(null, ""),
      cep: Joi.string().max(9).allow(null, ""),
      complemento: Joi.string().max(140).allow(null, ""),
      logadouro: Joi.string().max(140).allow(null, ""),
    },
...
const removeMiddleware = function (doc, next) {
  const company = doc;
  company.model("User").update({ company: company._id }, { $unset: { company: 1 } }, { multi: true }, next());
};
