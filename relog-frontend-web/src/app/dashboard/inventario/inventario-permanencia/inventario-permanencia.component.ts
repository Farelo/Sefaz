...
export class InventarioPermanenciaComponent implements OnInit {
  ...
  serialFilter(event: any) {
    ...
    if (!event) {
      filter = { family: this.selectedFamily._id }
    } else{
      filter = { family: this.selectedFamily._id, serial: this.selectedSerial }
    }

    this.reportService.getPermanenceInventory(filter).subscribe(result => {
      this.listOfPermanence = result;
    }, err => { console.log(err) });
  }
